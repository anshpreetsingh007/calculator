...
const ORANGE = '#F5922A';
const GRAPH_W = Dimensions.get('window').width - 48;
const GRAPH_H = 260;

const RANGE = 10;
const STEPS = 200;

function parseEquation(eq: string): ((x: number) => number) | null {
  let expr = eq.trim().toLowerCase();
  ...
  // swap common math notation to JS
  expr = expr
    .replace(/\^/g, '**')
  ...
  if (/[^0-9x+\-*/().,\s*MathsincotaglqrtbpPIE]/.test(expr)) {
    return null;
  }
...
export default function GraphCalc() {
  ...
  for (let i = -RANGE; i <= RANGE; i += 2) {
    gridLines.push(
      <Line key={`gv${i}`} x1={toScreenX(i)} y1={0} x2={toScreenX(i)} y2={GRAPH_H} stroke="#e0e0e0" strokeWidth={0.5} />,
      <Line key={`gh${i}`} x1={0} y1={toScreenY(i)} x2={GRAPH_W} y2={toScreenY(i)} stroke="#e0e0e0" strokeWidth={0.5} />
    );
  }
    ...
    <View style={s.container}>
      ...
      <View style={s.inputRow}>
        <TextInput
          style={s.eqInput}
          value={equation}
          onChangeText={(t) => { setEquation(t); setErr(''); }}
          placeholder="y=x"
      ...
      <View style={s.graphWrap}>
        <Svg width={GRAPH_W} height={GRAPH_H}>
          {gridLines}
          {/* axes */}
          <Line x1={toScreenX(0)} y1={0} x2={toScreenX(0)} y2={GRAPH_H} stroke="#999" strokeWidth={1} />
          <Line x1={0} y1={toScreenY(0)} x2={GRAPH_W} y2={toScreenY(0)} stroke="#999" strokeWidth={1} />

          {/* axis labels */}
          <SvgText x={GRAPH_W - 8} y={toScreenY(0) - 6} fontSize={10} fill="#999" textAnchor="end">x</SvgText>
          <SvgText x={toScreenX(0) + 6} y={14} fontSize={10} fill="#999">y</SvgText>
      ...
      <View style={s.funcRow}>
        ...
        <TouchableOpacity style={s.funcBtn} onPress={() => insertFunc('sin(x)')}>
          <Text style={s.funcText}>sin(x)</Text>
        </TouchableOpacity>
        <TouchableOpacity style={s.funcBtn} onPress={() => insertFunc('cos(x)')}>
          <Text style={s.funcText}>cos(x)</Text>
        </TouchableOpacity>
        <TouchableOpacity style={s.funcBtn} onPress={() => insertFunc('log(x)')}>
